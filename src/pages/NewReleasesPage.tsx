...
export const NewReleasesPage: React.FC = () => {
  ...
  // Fetch raw GameDB records directly when timeframe changes
  useEffect(() => {
    let isMounted = true;
    setLoading(true);

    fetchDirectGameDbReleases(timeframe).then(fetched => {
      if (isMounted) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      {/* Top Bar: Title + Day/Week/Month Toggle */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 glass-panel p-5 rounded-2xl border border-slate-800">
        <div>
          ...
          <span className="text-[10px] font-mono text-cyan-400 flex items-center gap-1 mt-0.5">
            ...
            Direct GameDB Query • {loading ? 'Fetching...' : `${liveGames.length} Games Found`}
          </span>
        </div>
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center p-12 glass-panel rounded-2xl border border-slate-800 space-y-3">
          ...
          <span className="text-xs font-mono text-slate-400">
            Querying LizardByte GameDB API directly for {timeframe.toUpperCase()} releases...
          </span>
        </div>
      )}

      {/* Rendered Direct GameDB Results */}
      {!loading && (
        <GameListGrid
          games={liveGames}
