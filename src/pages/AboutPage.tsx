...
import { Compass, Database, ExternalLink, Heart, MessageCircle, ShieldCheck } from 'lucide-react';

const officialSocialLinks = [
  { label: 'Bluesky', handle: '@playatlas.bsky.social', url: 'https://bsky.app/profile/playatlas.bsky.social' },
  { label: 'X / Twitter', handle: '@PlayAtlasX', url: 'https://x.com/PlayAtlasX' },
  { label: 'YouTube', handle: '@PlayAtlasOfficial', url: 'https://www.youtube.com/@PlayAtlasOfficial' },
  { label: 'Reddit', handle: 'u/PlayAtlasOfficial', url: 'https://www.reddit.com/user/PlayAtlasOfficial/' },
] as const;
...
export const AboutPage: React.FC = () => (
  <div className="mx-auto max-w-5xl space-y-5 animate-in fade-in duration-300">
    ...
    <div className="grid gap-5 md:grid-cols-2">
      <section className="atlas-dashboard-panel p-5 md:p-6"><div className="flex items-center gap-3"><Database className="h-5 w-5 text-[#8C6D37]" /><h2 className="font-serif text-2xl font-bold text-[#0C1D2D]">Catalog & privacy</h2></div><p className="mt-3 text-sm leading-relaxed text-[#47586A]">Public game information is sourced from IGDB. The event calendar combines Events for Gamers with IGDB enrichment. Personal ratings, notes, ownership, buckets, lists, completions, and calendar items remain in this browser’s local storage unless you explicitly export or share them.</p></section>
      <section className="atlas-dashboard-panel p-5 md:p-6"><div className="flex items-center gap-3"><MessageCircle className="h-5 w-5 text-[#8C6D37]" /><h2 className="font-serif text-2xl font-bold text-[#0C1D2D]">Join the community</h2></div><p className="mt-3 text-sm leading-relaxed text-[#47586A]">Share feedback, report catalog oddities, suggest features, or just talk games with the Play Atlas community.</p><a href="https://discord.gg/9vGY6kQTd" target="_blank" rel="noreferrer" className="mt-5 inline-flex items-center gap-2 rounded-xl border border-[#C5A059] bg-[#0B2B3C] px-4 py-2.5 text-sm font-bold text-white shadow-sm transition-transform hover:-translate-y-0.5"><MessageCircle className="h-4 w-4 text-[#C5A059]" /> Join the Play Atlas Discord</a><div className="mt-4 grid gap-2 sm:grid-cols-2">{officialSocialLinks.map(link => <a key={link.label} href={link.url} target="_blank" rel="noreferrer" className="flex items-center justify-between gap-2 rounded-xl border border-[#D9C8A9] bg-[#FDFBF7] px-3 py-2.5 transition-colors hover:bg-[#EFE8D8]"><span className="min-w-0"><span className="block text-xs font-bold text-[#0C1D2D]">{link.label}</span><span className="block truncate text-[10px] font-semibold text-[#47586A]">{link.handle}</span></span><ExternalLink className="h-3.5 w-3.5 shrink-0 text-[#8C6D37]" /></a>)}</div></section>
    </div>

    <section className="atlas-dashboard-panel p-5 md:p-6"><div className="flex items-center gap-3"><ShieldCheck className="h-5 w-5 text-[#8C6D37]" /><h2 className="font-serif text-2xl font-bold text-[#0C1D2D]">Credits & trademarks</h2></div><div className="mt-3 space-y-3 text-xs leading-relaxed text-[#47586A]"><p>Game metadata is provided through the IGDB API, and event information is provided by Events for Gamers with IGDB enrichment. IGDB is operated by Twitch. Play Atlas is an independent project and is not endorsed by, affiliated with, or sponsored by Events for Gamers, IGDB, Twitch, game publishers, platform holders, or storefront operators.</p><p>All game titles, artwork, logos, platform names, company names, and related trademarks are the property of their respective owners. Their appearance is for identification and informational purposes.</p><p>Play Atlas™ is the name and original branding of this independent project.</p></div></section>

    <section className="atlas-dashboard-panel flex flex-col items-center p-6 text-center"><Heart className="h-6 w-6 text-rose-600" /><p className="mt-3 max-w-2xl font-serif text-xl font-semibold italic text-[#0C1D2D]">Built as the gaming organizer I wanted, when none were seemingly available. If you want something done right...</p><p className="mt-3 text-sm font-bold text-[#8C6D37]">— LukieSeven</p></section>
  </div>
);
