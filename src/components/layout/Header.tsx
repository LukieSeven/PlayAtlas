...
export const Header: React.FC<HeaderProps> = ({ onSelectGame }) => {
  ...
  return (
    <header className="sticky top-0 z-30 themed-header w-full px-4 md:px-8 py-3 flex items-center justify-between gap-4 border-b border-[var(--panel-border)] shadow-sm bg-[#f5f0e1]">
      ...
      <div className="relative flex-1 max-w-xl" ref={searchRef}>
        ...
        {searchQuery.trim().length >= 2 && (
          <div className="absolute top-full left-0 right-0 mt-2 themed-panel rounded-2xl border border-[var(--panel-border)] shadow-2xl overflow-hidden z-50 max-h-96 overflow-y-auto bg-[#fefcf6]">
            {isSearching ? (
              <div className="p-6 text-center text-xs text-[#0f2b48] flex items-center justify-center gap-2 font-mono">
                <Loader2 className="w-4 h-4 animate-spin text-[var(--primary-action)]" />
                <span>Searching Play Atlas catalog...</span>
              </div>
            ) : results.length === 0 ? (
              <div className="p-6 text-center text-xs text-[#475569] font-mono">
                No matching games found for "{searchQuery}".
              </div>
            ) : (
              <div className="divide-y divide-[var(--panel-border)]">
                <div className="px-4 py-2 bg-[#ece4d0] text-[10px] font-mono text-[#0f2b48] font-bold uppercase flex justify-between">
                ...
                {results.map((game: CompactGameLookupRecord) => {
                  ...
                  return (
                    <div
                      ...
                      className="p-3 hover:bg-[#ece4d0]/60 cursor-pointer flex items-center justify-between gap-3 transition-colors"
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-2">
                            <h4 className="font-bold text-xs text-[#0f2b48] truncate">
                              {game.name}
                            </h4>
                            {showTypeBadge && (
                              <span className="px-1.5 py-0.5 rounded text-[9px] font-extrabold bg-cyan-500/20 text-cyan-800 border border-cyan-500/30 uppercase">
      ...
      <div className="flex items-center gap-3" ref={themeRef}>
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 rounded-2xl themed-panel border border-[var(--panel-border)] hover:border-[var(--accent-color)] text-xs font-bold transition-all shadow-sm bg-[#fefcf6] text-[#0f2b48]"
            title="Change Visual Theme"
          >
            <Palette className="w-4 h-4 text-[var(--accent-color)]" />
            <span className="hidden sm:inline text-[#0f2b48]">{activeTokens.name.split(' ')[0]}</span>
            <div
          ...
          {isThemePopoverOpen && (
            <div className="absolute right-0 top-full mt-2 w-64 themed-panel rounded-2xl border border-[var(--panel-border)] shadow-2xl p-3 z-50 animate-in fade-in zoom-in-95 duration-150 space-y-2 bg-[#fefcf6] text-[#0f2b48]">
